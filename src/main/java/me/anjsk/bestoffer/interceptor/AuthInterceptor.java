...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        // 호출하려는 메서드에 @RequireAdmin, @RequireLogin가 붙어있는지 확인
        boolean isLoginRequired = handlerMethod.hasMethodAnnotation(RequireLogin.class);
        boolean isAdminOnly = handlerMethod.hasMethodAnnotation(RequireAdmin.class);

        // 로그인이 필요하지 않다면, 누구나 접근 가능
        if (!isLoginRequired && !isAdminOnly) {
            return true;
        }

        // 로그인이 필요하다면, 세션을 꺼내서 검사 시작
